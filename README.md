# 👋 Olá! Bem-vindo ao meu perfil!

## 🚀 Sobre Mim

Sou um desenvolvedor e especialista em **Gestão de Redes** apaixonado por **tecnologia, automação e soluções criativas**. Trabalho com diversos projetos que envolvem **containerização, infraestrutura, games e ferramentas de sistema**. Meu foco é criar soluções práticas e eficientes que resolvem problemas reais.

---

## 💼 Meus Projetos

### 🐳 **Docker & Containerização**

- **[Windows-inside-a-Docker-container](https://github.com/duenke324/Windows-inside-a-Docker-container)** - Windows rodando dentro de um container Docker 🟦

### 🎮 **Minecraft & Gaming**

- **[ServerPackCreator](https://github.com/duenke324/ServerPackCreator)** - Ferramenta para criar server packs de modpacks Minecraft (Forge, NeoForge, Fabric, Quilt)
  - 🔗 [Visite o site do projeto](https://serverpackcreator.de)

---

## 🛠️ Tecnologias & Ferramentas

- 🌐 **Gestão de Redes** - Configuração, monitoramento e otimização
- 🐳 **Docker** - Containerização e orquestração
- 🔧 **Automation** - Scripts e ferramentas customizadas
- 💻 **Windows & Linux** - Desenvolvimento cross-platform
- 🎮 **Minecraft Modding** - Forge, Fabric, Quilt
- 📦 **DevOps** - Infraestrutura e deployment
- 🔧 **FiveM** - Scripts e ferramentas customizadas

---

## 📊 Estatísticas

![GitHub stats](https://github-readme-stats.vercel.app/api?username=duenke324&show_icons=true&theme=dark)

---

## 📫 Me Encontre

- 🔗 GitHub: [@duenke324](https://github.com/duenke324)

---

### ⭐ Se você gostou dos meus projetos, considere dar uma estrela! 

**Contribuições e feedback são sempre bem-vindos!** 🚀
